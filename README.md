...
2. **规模与成本经济** — 章节并行、子代理逐章翻译;已完成工作不重复付费(DSH 会话记录承接);反思修订按书询问、按需开启。

**当前状态**:设计定稿,仓库架构与开发流程已建立。确定性文本引擎(`core`)已落地 slug、章节识别(`##` 章边界)、Markdown 空行分段与全书组装(D56/D57);`tools`(工具面)与 `client`(UI)为构建管线骨架,业务代码随里程碑推进。尚未链入 DSH 部署。
...
- [ ] `tools` 工具面与书级状态文件;preset(`~/.dsh/.agent-presets/itranslation/`)链入本机部署(D15)
- [ ] `client` UI(Run 卡进度、设置页)
