...
- D58 工具面定案：`@deepseek-ai/dsh-itranslation-tools` 以 Cordis 插件形态导出 `name`/`inject`(`tools`,`fs`)/`Config`/`apply`，经 `ctx.tools.register(defineTool(...))` 注册六个确定性工具（§5.4 候选名定稿）：`itranslation_prepare`（读 Markdown→normalize/detectChapters→落盘 `source/<n>.md`+`state.json`，书名缺省取自文件名，`state.json` 已存在即拒绝防覆盖）、`itranslation_segment`（只读报告：按章段数/句数/字节，超长章标记，不写盘）、`itranslation_glossary`（`glossary.json` 的 upsert/remove 回写）、`itranslation_align`（读 source+chapters 含 `<n>.<k>` 分片→core `assembleBook` 组装校验→写 `aligned.md` 预览）、`itranslation_assemble`（前置 `state.json`+`chapters/`+`audit-report.md`→重组装→写成品 `books/<slug>/<slug>.md`+`meta.json`）、`itranslation_status`（只读进度与证据摘要）。书级目录 `<cwd>/books/<slug>/`（cwd=`exec.agent.session.header.cwd`，D31/D46）。步骤顺序/前置校验硬拦抛错（D38）；对齐失配等数据条件返回结构化 `ok:false`+`mismatch` 结果由 agent 转问用户（D56），不抛。`state.json` 即 core `BookState`（`{title,chapters:[{index,title}]}`，不增字段）；`meta.json` 记录 `schemaVersion`/`engineVersion`/书名/slug/目录/章段数/成品路径/`assembledAt`/`processes[]`（LLM 过程记录由 agent 收集后经 `assemble` 的 `processes` 入参写入，D29/D34）；`glossary.json` 为 `{entries:[{term,translation,note?,source?}]}`。
...
- D61 client UI 定案：`@deepseek-ai/dsh-itranslation-client` 浏览器半边按真实 DSH slot 落地——Run 卡进度经 keyed `tool.call.toolview` 为六个 `itranslation_*` 工具各注册一行（从冻结 call/result 派生中文摘要，含运行中/失败/解析失败/失配/进度）；设置页经 `settings.section`（id `itranslation`）读写 `itranslation` 命名空间中的体裁默认与术语确认模式，命名空间缺失/只读/保存失败均降级为可读提示。client 包 peer 依赖以 `link:` 指向 `~/deepseek-harness` 已构建包目录（D59 同法），浏览器 bundle 用 tsdown 输出 `lib/client.js`（CJS + `window.__ModuleLoader__.load` 手写头尾，`react`/`@deepseek-ai/dsh-client-web-react` 走平台模块表 external）。
- D62 设置命名空间注册定案：`itranslation` 命名空间由 `@deepseek-ai/dsh-itranslation-client` 的 Host 半边在 web plane 注册（D68 起；此前为 tools 包注册）：`apply` 中经 `ctx.inject(['settings'])` 注册，schema 为四个 LLM 提示词（`preReadPrompt`/`translatePrompt`/`auditPrompt`/`revisePrompt`，默认空串，D65），与 client 设置页读取同一命名空间；无 settings provider 时注入不生效、不阻断加载。

- D63 链入状态实录(2026-08-19 验证)：preset 目录 `~/.dsh/.agent-presets/itranslation/` 与仓库 `presets/itranslation/` 一致、已装到位;`agent.cordis.yml:227` 的 `itranslation-tools` 行以绝对路径挂载,经 Cordis loader(`tree.import` 直交 Node import)实测加载成功并注册 `itranslation_prepare/itranslation_segment/itranslation_glossary/itranslation_align/itranslation_assemble/itranslation_status` 六工具,host 工具侧链入成立(临时验证形态,D60);client 浏览器半边未进部署模块表(`__DSH_BOOT__.entries` 无 `@deepseek-ai/dsh-itranslation-client`);其 host 半边原仅导出常量、缺 Cordis `apply` 会被 Loader 拒为 entry,已按 harness `dsh-client-ui-tool` 范式补 no-op `apply` 并加测(D63 修正),同日链入生效见 D64。工具名统一使用下划线并符合 DeepSeek API 的工具名正则 `^[a-zA-Z0-9_-]+$`（D67）;README/AGENTS 的「尚未链入/待后续」表述据此修正。
- D64 链入完成实录(2026-08-19)：client 浏览器 bundle 原以 tsdown `intro` 在 bundle 顶层声明 `var module/exports`,在 `window.__ModuleLoader__.load` 的 factory 语义下落到错误作用域;已改为 `banner` 的 `factory(require)` 闭包内声明(`var module={exports:{}}; var exports=module.exports;`)、`footer` 返回 `module.exports`,web 端加载通过。部署链入脚本 `scripts/install-web-deploy.mjs`(幂等,dry-run/--apply):`--apply` 写 `~/.dsh/profiles/web/package.json`(file: 依赖指向 `packages/itranslation/client`)与 `cordis.patch.yml`(insert `@deepseek-ai/dsh-itranslation-client` entry),随后 `pnpm install` + 重启 `dsh web`。至此 D15「包链入本机部署」完成:host 六工具与 client Run 卡/设置页在 3080 可用;下一里程碑为九步流程端到端冒烟 + `meta.json` 证据链(D39)。
- D65 交互与 LLM 环节定案(2026-08-19)：停点①只问目标语言,其余(体裁/风格/术语模式)全自动;预读与统一风格合并——预读子代理直接产出正式 `style.md` 与 `glossary.json`,LLM 环节从五处减为四处(预读/翻译/审查/修订);停点②改为 agent 生成 `glossary.json` 后停下,用户自行编辑文件后确认继续;设置页改为四个提示词文本框(预读/翻译/审查/修订,空默认值,不预设内容),settings 命名空间 schema 同步为 `{preReadPrompt,translatePrompt,auditPrompt,revisePrompt}`;preset persona 写入上述流程指令。
- D66 Client UI 视觉定案(2026-08-20)：client 设置页必须对齐 harness 设计语言,不得用裸浏览器默认样式——组件从 `@deepseek-ai/dsh-client-ui-primitives`(Button/Input 等共享原子)组合,样式走 `*.module.css` 且颜色全部解析到 `--dsw-alias-*` token(禁用应用未定义的裸 `--border`/`--surface`/`--text-*` 名),浏览器 bundle 经共享 `clientBundle` tsdown preset 产出(内置 CSS Module 内联插件),禁止手写 `className` 字符串或手写绕开该管线的 tsdown 配置。client 包为此须补 `@deepseek-ai/dsh-client-ui-primitives`/`lightningcss`/`clsx` 依赖与 `css-modules.d.ts` 声明。此条记录本次「设置页渲染为默认样式、与其他插件不一致」的根因,源: DSH-pr plugin-requirements §4。
- D67 工具名兼容：六个模型可见工具统一使用下划线命名（`itranslation_prepare`、`itranslation_segment`、`itranslation_glossary`、`itranslation_align`、`itranslation_assemble`、`itranslation_status`），满足 DeepSeek API 工具名正则 `^[a-zA-Z0-9_-]+$`。

### 2026-08-21

- D68 设置命名空间归属变更：web 设置页报「命名空间未检测」的根因是注册方在 agent plane——tools 包仅在 itranslation 会话挂载时加载，其 `settings.register`（tools 行无 realm、消费宿主注册表）落在与 web 共享的 host-root settings 实例上，会话不存在即缺失（`~/.dsh/settings.yaml` 亦无分节）。定案：注册归属移至 client 包 Host 半边——client 经 cordis.patch.yml 常驻 web plane，命名空间与会话生命周期解耦；tools 包移除注册（同一共享实例上重复注册会在会话挂载时立即报错）。client 包为此补 `@deepseek-ai/dsh-settings`/`@deepseek-ai/schemastery` peer + dev 依赖（link 指向 harness，D59 同法）。
- D69 设置页数据通道变更：D68 后设置页仍不可用，根因是**浏览器 settings 线被 apiproxy 白名单过滤**——`packages/host/apiproxy` 的 `settings.describe`/`update` 只放行 `WEB_SETTINGS_NAMESPACES` + 模型提供方 + `ui-onboarding`/`agent-presets`，注释明言插件自暴露是 harness 的 deferred work；第三方插件命名空间经该线永远不可达。定案：设置页改走**插件自有同源路由** `/_dsh/itranslation/settings`（vision-toolkit 的 `/_dsh/<plugin>/...` 先例）——client host 半边经 `webServer.register` 挂载路由：GET 服务端 `ctx.settings.describe` 快照（value/writable/hasDocument/revision），POST 服务端 `settings.update`（同源守卫、`SettingsConflictError`→409、其余拒绝→400/503）；浏览器 store 由 `connection.api.settings.*` 改为 fetch 读写该路由（`createItranslationSettingsApi`），后端缺失（`settings-unavailable`）降级为只读提示。client 包补 `@deepseek-ai/dsh-host-webserver` peer + dev 依赖；设置页缺省文案由「命名空间未检测」改为「设置后端不可用」。client 包 tsconfig 依 harness api-remotes 先例分裂为 host/client 双 face（host face 含 node 类型，编译 web 路由；client face 保持浏览器纯度 `types: []`），根聚合分别引用对应 face。
- D70 目录接入定案：输入输出改由会话 workspace 下的固定文件夹接入——用户把 E2M 转出的 Markdown 放入 `<cwd>/input/`（`prepare` 的 `path` 传 `input/<文件>.md`，E2M 仍为流程外外部步骤、未入码，D55）；`books/<slug>/` 书级工作目录**全部保留**（state/source/chapters/audit/meta 等不变）；最终成品由 `assemble` 写入 `<cwd>/output/<slug>.md`（`outputRel` 从 `books/<slug>/<slug>.md` 迁出，books/ 中不再含成品；`meta.json` 的 `outputFile` 记录新路径）。persona 与 DESIGN §5.5 同步。
