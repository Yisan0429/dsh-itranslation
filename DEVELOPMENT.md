...
- D7 子代理：一章一个 `spawn` 子代理（独立上下文、模型可覆盖、不占主上下文），风格+术语随任务注入；并发/分批交主 agent 自定，插件不设上限。
- D8 审查：全书译完统一审，由独立"审查模型"（第 0 步自选，可不同于主 agent 模型）执行。
- D9 反思：只修具体问题，不按章重跑；开关每本书问。
- D10 输出格式：第 0 步由用户选。
- D11 横切：不要成本闸；断点续跑不另建（见 D23）。
- D12 引擎：DSH 原生实现，无 Python 依赖。
- D13 模型：预读、翻译、审查、修订四个过程由用户自选模型；统一风格由主 agent 模型承担。
...
- D21 格式策略：删除格式型里程碑；输入/输出格式不限、用户自由选择；统一转 Markdown、原格式保留存档；转换由 agent 调用工具完成；格式适配层为 TS 工具随里程碑补齐，未适配格式运行时提示暂不支持。
- D22 分段定案：插件按段落确定性分段；主 agent 仅调整异常段并留痕；最终清单落盘；子代理照单逐段翻译。
- D23 断点：DSH 会话本身持久记录全部过程，中断不影响记录；无需自建断点续跑；恢复=回到会话继续，跨会话按书级目录文件核对进度。
...
3. **证据闸（里程碑级）**：`tools`/`core` 包行为改动以单元测试为准；每个里程碑验收时用样本短书跑通九步冒烟并产出 `meta.json`，存档（路径按 `DESIGN.md` §5.5）。
