...
- D3 交付：只出设计文档，暂不实现。
- D4 流程八步：确认要求 → 提取预读 → 风格+术语 → 按章节分段 → 子代理逐章翻译 → 对齐组装 → 全书审查 → 反思修订 → 出成品。
...
- D19 审查标准：交子代理调研（参考书籍编译/出版行业），提案经用户确认后回填 DESIGN §4。
- D20 审查报告不分级：只列问题清单，修不修由用户停点③决定。
- D21 格式策略：删除格式型里程碑；输入格式用户自选；统一转 Markdown 处理、原格式保留；输出按用户选择。
