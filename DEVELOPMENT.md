...
- D23 分块定案：插件不预分块；第 3 步收窄为章节识别；子代理自选分段翻译、边译边落盘；断点=章；审查定位到句靠模型引用原文。
- D26 断点机制取消（取代 D24，D11 相关部分作废）：DSH 会话本身持久记录全部过程，中断不影响会话记录；无需插件自建断点续跑；恢复=回到会话继续，跨会话按书级目录文件核对进度。
- D25 分段权修订（取代 D23 相关部分）：插件按段落确定性分段；主 agent 仅可调整异常段并留痕；最终清单落盘；子代理照单逐段翻译。
